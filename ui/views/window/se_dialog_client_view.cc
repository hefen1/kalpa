...
#include "ui/views/window/se_dialog_client_view.h"
...
///////////////////////////////////////////////////////////////////////////////
// SeDialogClientView, public:

SeDialogClientView::SeDialogClientView(Widget* owner, View* contents_view)
...
SeDialogClientView::~SeDialogClientView() {
}

void SeDialogClientView::AcceptWindow() {
...
void SeDialogClientView::CancelWindow() {
  // Only notify the delegate once. See |notified_delegate_|'s comment.
...
void SeDialogClientView::UpdateDialogButtons() {
...
///////////////////////////////////////////////////////////////////////////////
// SeDialogClientView, ClientView overrides:

bool SeDialogClientView::CanClose() {
...
SeDialogClientView* SeDialogClientView::AsSeDialogClientView() {
  return this;
}

const SeDialogClientView* SeDialogClientView::AsSeDialogClientView() const {
  return this;
}

void SeDialogClientView::OnWillChangeFocus(View* focused_before,
                                         View* focused_now) {
  ...
  ImageTextButton* new_default_button = NULL;
  if (focused_now &&
      !strcmp(focused_now->GetClassName(), ImageTextButton::kViewClassName)) {
    new_default_button = static_cast<ImageTextButton*>(focused_now);
...
void SeDialogClientView::OnDidChangeFocus(View* focused_before,
                                        View* focused_now) {
}

////////////////////////////////////////////////////////////////////////////////
// SeDialogClientView, View overrides:

gfx::Size SeDialogClientView::GetPreferredSize() {
...
void SeDialogClientView::Layout() {
  ...
  if (has_dialog_buttons() || ShouldShow(extra_view_)) {
    ...
    if (cancel_button_) {
      ...
      cancel_button_->SetBounds(row_bounds.right() + 17, row_bounds.y() + 18,
                                size.width(), height);
    ...
    if (ok_button_) {
      ...
      row_bounds.set_width(row_bounds.width() - size.width());
      ok_button_->SetBounds(row_bounds.right() + 17, row_bounds.y() + 18,
                            size.width(), height);
      row_bounds.set_width(row_bounds.width() - kRelatedButtonHSpacing);
    }
    if (extra_view_) {
...
bool SeDialogClientView::AcceleratorPressed(const ui::Accelerator& accelerator) {
  DCHECK_EQ(accelerator.key_code(), ui::VKEY_ESCAPE);
  Close();
  return true;
}

void SeDialogClientView::ViewHierarchyChanged(
...
void SeDialogClientView::NativeViewHierarchyChanged() {
...
////////////////////////////////////////////////////////////////////////////////
// SeDialogClientView, ButtonListener implementation:

void SeDialogClientView::ButtonPressed(Button* sender, const ui::Event& event) {
...
SeDialogClientView::SeDialogClientView(View* contents_view)
...
DialogDelegate* SeDialogClientView::GetDialogDelegate() const {
  return GetWidget()->widget_delegate()->AsDialogDelegate();
}

void SeDialogClientView::CreateExtraView() {
  if (extra_view_)
    return;
...
void SeDialogClientView::CreateFootnoteView() {
  if (footnote_view_)
    return;
...
void SeDialogClientView::ChildPreferredSizeChanged(View* child) {
  if (child == footnote_view_ || child == extra_view_)
    Layout();
}

void SeDialogClientView::ChildVisibilityChanged(View* child) {
  ChildPreferredSizeChanged(child);
}

////////////////////////////////////////////////////////////////////////////////
// SeDialogClientView, private:

ImageTextButton* SeDialogClientView::CreateDialogButton(ui::DialogButton type) {
  ...
  if (GetDialogDelegate()->UseNewStyleForThisDialog() &&
     ...
     button = new ImageTextButton(this, title);
  } else {
    button = new ImageTextButton(this, title);
    ui::ResourceBundle& rb = ui::ResourceBundle::GetSharedInstance();
    if (type == GetDialogDelegate()->GetDefaultDialogButton()) {
      button->SetFontColor(SK_ColorWHITE);
      button->SetImage(views::Button::STATE_NORMAL, rb.GetImageSkiaNamed(IDR_SE_DIALOG_OK_BTN_N));
      button->SetImage(views::Button::STATE_PRESSED, rb.GetImageSkiaNamed(IDR_SE_DIALOG_OK_BTN_P));
      button->SetImage(views::Button::STATE_HOVERED, rb.GetImageSkiaNamed(IDR_SE_DIALOG_OK_BTN_H));
    } else {
      button->SetFontColor(SK_ColorBLACK);
      button->SetImage(views::Button::STATE_NORMAL, rb.GetImageSkiaNamed(IDR_SE_DIALOG_CANCLE_BTN_N));
      button->SetImage(views::Button::STATE_PRESSED, rb.GetImageSkiaNamed(IDR_SE_DIALOG_CANCLE_BTN_P));
      button->SetImage(views::Button::STATE_HOVERED, rb.GetImageSkiaNamed(IDR_SE_DIALOG_CANCLE_BTN_H));
    }
  }
  button->set_focusable(true);
  button->set_focus_border(NULL);
  button->SetGroup(kButtonGroup);
  return button;
}

void SeDialogClientView::UpdateButton(ImageTextButton* button,
...
int SeDialogClientView::GetButtonsAndExtraViewRowHeight() const {
...
gfx::Insets SeDialogClientView::GetButtonRowInsets() const {
