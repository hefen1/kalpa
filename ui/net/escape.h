...
UI_EXPORT std::string EscapeQueryParamValue(const std::string& text,
                                             bool use_plus);
...
UI_EXPORT std::string EscapePath(const std::string& path);
...
// as %XX (hex).
UI_EXPORT std::string EscapeUrlEncodedData(const std::string& path,
                                            bool use_plus);

// Escapes all non-ASCII input.
UI_EXPORT std::string EscapeNonASCII(const std::string& input);
...
// chracters (;/?:@&=+$,).
UI_EXPORT std::string EscapeExternalHandlerValue(const std::string& text);
...
UI_EXPORT void AppendEscapedCharForHTML(char c, std::string* output);

// Escapes chars that might cause this text to be interpretted as HTML tags.
UI_EXPORT std::string EscapeForHTML(const std::string& text);
UI_EXPORT base::string16 EscapeForHTML(const base::string16& text);
...
UI_EXPORT std::string UnescapeURLComponent(const std::string& escaped_text,
                                            UnescapeRule::Type rules);
UI_EXPORT base::string16 UnescapeURLComponent(
...
UI_EXPORT base::string16 UnescapeAndDecodeUTF8URLComponent(
    const std::string& text,
    UnescapeRule::Type rules,
    size_t* offset_for_adjustment);
UI_EXPORT base::string16 UnescapeAndDecodeUTF8URLComponentWithOffsets(
    const std::string& text,
    UnescapeRule::Type rules,
...
// &lt; &gt; &amp; &quot; &#39;
UI_EXPORT base::string16 UnescapeForHTML(const base::string16& text);
