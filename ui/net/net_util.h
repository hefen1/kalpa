...
// Nothing is ommitted.
UI_EXPORT extern const FormatUrlType kFormatUrlOmitNothing;

// If set, any username and password are removed.
UI_EXPORT extern const FormatUrlType kFormatUrlOmitUsernamePassword;

// If the scheme is 'http://', it's removed.
UI_EXPORT extern const FormatUrlType kFormatUrlOmitHTTP;

// Omits the path if it is just a slash and there is no query or ref.  This is
// meaningful for non-file "standard" URLs.
UI_EXPORT extern const FormatUrlType kFormatUrlOmitTrailingSlashOnBareHostname;

// Convenience for omitting all unecessary types.
UI_EXPORT extern const FormatUrlType kFormatUrlOmitAll;
/*
...
UI_EXPORT GURL FilePathToFileURL(const base::FilePath& path);
...
UI_EXPORT bool FileURLToFilePath(const GURL& url, base::FilePath* file_path);
...
UI_EXPORT bool ParseHostAndPort(
...
UI_EXPORT std::string GetHostAndPort(const GURL& url);
...
UI_EXPORT bool IsHostnameNonUnique(const std::string& hostname);
...
UI_EXPORT bool IsIPAddressReserved(const IPAddressNumber& address);
...
UI_EXPORT std::string IPAddressToString(const uint8* address,
                                         size_t address_len);

// Returns the string representation of an IP address along with its port.
// For example: "192.168.0.1:99" or "[::1]:80".
UI_EXPORT std::string IPAddressToStringWithPort(const uint8* address,
                                                 size_t address_len,
                                                 uint16 port);

// Same as IPAddressToString() but for a sockaddr. This output will not include
// the IPv6 scope ID.
UI_EXPORT std::string NetAddressToString(const struct sockaddr* sa,
                                          socklen_t sock_addr_len);

// Same as IPAddressToStringWithPort() but for a sockaddr. This output will not
// include the IPv6 scope ID.
UI_EXPORT std::string NetAddressToStringWithPort(const struct sockaddr* sa,
                                                  socklen_t sock_addr_len);

// Same as IPAddressToString() but for an IPAddressNumber.
UI_EXPORT std::string IPAddressToString(const IPAddressNumber& addr);

// Same as IPAddressToStringWithPort() but for an IPAddressNumber.
UI_EXPORT std::string IPAddressToStringWithPort(
    const IPAddressNumber& addr, uint16 port);

// Returns the address as a sequence of bytes in network-byte-order.
UI_EXPORT std::string IPAddressToPackedString(const IPAddressNumber& addr);

// Returns the hostname of the current system. Returns empty string on failure.
UI_EXPORT std::string GetHostName();
...
UI_EXPORT std::string GetHostOrSpecFromURL(const GURL& url);
...
UI_EXPORT std::string GetSpecificHeader(const std::string& headers,
...
UI_EXPORT base::string16 IDNToUnicode(const std::string& host,
                                 const std::string& languages);
*/
// Canonicalizes |host| and returns it.  Also fills |host_info| with
// IP address information.  |host_info| must not be NULL.
UI_EXPORT std::string CanonicalizeHost(const std::string& host,
...
UI_EXPORT bool IsCanonicalizedHostCompliant(const std::string& host,
                                             const std::string& desired_tld);

// Call these functions to get the html snippet for a directory listing.
// The return values of both functions are in UTF-8.
UI_EXPORT std::string GetDirectoryListingHeader(const base::string16& title);
...
UI_EXPORT std::string GetDirectoryListingEntry(const base::string16& name,
...
// unmodified.
UI_EXPORT base::string16 StripWWW(const base::string16& text);
/*
// Runs |url|'s host through StripWWW().  |url| must be valid.
UI_EXPORT base::string16 StripWWWFromHost(const GURL& url);
...
*/
UI_EXPORT base::string16 GetSuggestedFilename(
    const GURL& url,
...
UI_EXPORT base::FilePath GenerateFileName(
    const GURL& url,
    const std::string& content_disposition,
    const std::string& referrer_charset,
    const std::string& suggested_name,
...
// * do not end with ' ' or '.'
UI_EXPORT bool IsSafePortablePathComponent(const base::FilePath& component);
...
// absolute paths.
UI_EXPORT bool IsSafePortableRelativePath(const base::FilePath& path);
...
// thread that allows IO.
UI_EXPORT void GenerateSafeFileName(const std::string& mime_type,
...
UI_EXPORT bool IsPortAllowedByDefault(int port);
...
*/
UI_EXPORT base::string16 FormatUrl(const GURL& url,
                                    const std::string& languages,
...
UI_EXPORT base::string16 FormatUrlWithOffsets(
    const GURL& url,
    const std::string& languages,
    FormatUrlTypes format_types,
    UnescapeRule::Type unescape_rules,
...
UI_EXPORT bool CanStripTrailingSlash(const GURL& url);
/*
...
UI_EXPORT void SetExplicitlyAllowedPorts(const std::string& allowed_ports);

class UI_EXPORT ScopedPortException {
 public:
...
//    a:b:c::/46
//    ::1/128
UI_EXPORT bool ParseCIDRBlock(const std::string& cidr_literal,
                               IPAddressNumber* ip_number,
...
// interface.
struct UI_EXPORT NetworkInterface {
  NetworkInterface();
...
UI_EXPORT bool GetNetworkList(NetworkInterfaceList* networks);
...
UI_EXPORT WifiPHYLayerProtocol GetWifiPHYLayerProtocol();
*/
}  // namespace net
